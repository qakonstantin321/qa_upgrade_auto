...
class TransferMoneyPage(BasePage):
    ...
    def recipient_account_number_input(self, account_id: int):
        return self.page.locator("input[placeholder='Enter recipient account number']").fill(f"ACC{account_id}")
    ...
    def transfer(self, transfer_money_request: TransferMoneyRequest, name: str):
        self.account_select_by_id(transfer_money_request.senderAccountId)
        self.recipient_name_input(name)
        self.recipient_account_number_input(transfer_money_request.receiverAccountId)
